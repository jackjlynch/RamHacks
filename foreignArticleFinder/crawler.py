        ...
        if self.source.type == "rss":
            for article in  feedparser.parse(self.source.url)["entries"]:
                if not Article.objects.filter(title=article['title']).exists():
                    article = Article(language=self.source.language, source=self.source,
                                      text=article['summary_detail']['value'], title=article['title'],
                                      author=article['author'], url=article['link'], pub_date=article['published'])
