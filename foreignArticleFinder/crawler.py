        ...
        if self.source.type == "rss":
            for article in  feedparser.parse(self.source.url)["entries"]:
                if not Article.objects.filter(title=article['title']).exists():
                    Article(language=self.source.language, source=self.source, text=article['summary_detail'], title=article['title'], author=article['author'], url=article['link'], pub_date=article['published']).save()